...
struct DSActivityIndicatorTest {
    ...
    func testJsonMapperImpl() {
        ...
        guard let viewModel = my_view as? DSActivityIndicatorViewModel else {
            #expect(false, "my_view is not a DSActivityIndicatorViewModel")
            return
        }
        
        #expect(viewModel.identifier == "my_activityIndicator", "id should be 'my_activityIndicator'")
        #expect(viewModel.style == .primary, "Style should be 'primary'")
        #expect(viewModel.size == .large, "Size should be 'large'")
        #expect(viewModel.state == .running, "State should be 'running'")
