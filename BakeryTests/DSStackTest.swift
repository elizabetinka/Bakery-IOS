...
struct DSStackTest {
    ...
    func testJsonMapperImpl() {
        ...
        guard let viewModel = my_view as? DSStackViewModel else {
            #expect(false, "my_view is not a DSStackViewModel")
            return
        }

        #expect(viewModel.identifier == "my-stack", "id should be 'my-stack'")
        #expect(viewModel.alignment == .equal, "Style should be 'equal'")
        #expect(viewModel.size == .m, "Size should be 'm'")
        #expect(viewModel.style == .vertical, "State should be 'vertical'")
